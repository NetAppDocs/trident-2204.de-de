...
* Verbessertes Analysieren von iSCSI-Initiatornamen. (Ausgabe [#681]
* Das Problem wurde behoben, bei dem CSI-Speicherklassenparameter nicht zulässig waren. (Ausgabe [#598]
* Doppelte Schlüsseldeklaration im Trident CRD behoben. (Ausgabe [#671]
* Fehlerhafte CSI-Snapshot-Protokolle wurden korrigiert. (Ausgabe [#629]
* Problem beim Aufheben der Veröffentlichung von Volumes auf gelöschten Nodes behoben. (Ausgabe [#691]
* Zusätzliche Bearbeitung von Inkonsistenzen im Dateisystem auf Blockgeräten. (Ausgabe [#656]
* Problem beim Ziehen von Bildern mit automatischer Unterstützung beim Einstellen des behoben `imageRegistry` Markierung während der Installation. (Ausgabe [#715]
...
* Eingehende Verbindungen zu den sicheren Endpunkten von Trident erfordern jetzt mindestens TLS 1.3. (Ausgabe [#698]
...
* *Kubernetes*: Trident Demonset wird jetzt in der Klasse mit System-Node-kritischer Priorität ausgeführt. (Ausgabe [#694]
